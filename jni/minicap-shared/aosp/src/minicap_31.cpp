...
minicap_try_get_display_info(int32_t displayId, Minicap::DisplayInfo* info) {
  ...
  android::status_t err = android::SurfaceComposerClient::getDisplayState(dpy, &displayState);
  ...
  err = android::SurfaceComposerClient::getActiveDisplayMode(dpy, &displayMode);
  if (err != android::NO_ERROR) {
    MCERROR("SurfaceComposerClient::getActiveDisplayMode() failed: %s (%d)\n", error_name(err), err);
    return err;
  }
  
  android::ui::StaticDisplayInfo staticDisplayInfo;
  err = android::SurfaceComposerClient::getStaticDisplayInfo(dpy, &staticDisplayInfo);
  if (err != android::NO_ERROR) {
    MCERROR("SurfaceComposerClient::getStaticDisplayInfo() failed: %s (%d)\n", error_name(err), err);
    return err;
  }

  const android::ui::Size& layerStackSpaceRect = displayState.layerStackSpaceRect;
  MCINFO("Display is %dx%d @%.2ffps (orientation=%s), layerStack=%u\n",
  ...
  info->width = layerStackSpaceRect.getWidth();
  info->height = layerStackSpaceRect.getHeight();
  info->orientation = toRotationInt(displayState.orientation);
  info->fps = displayMode.refreshRate;
  info->density = staticDisplayInfo.density;
  info->xdpi = displayMode.xDpi;
  info->ydpi = displayMode.yDpi;
  info->secure = staticDisplayInfo.secure;
  info->size = sqrt(pow(info->width / info->xdpi, 2) + pow(info->height / info->ydpi, 2));
