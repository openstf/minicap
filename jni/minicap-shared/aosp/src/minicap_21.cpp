...
public:
  ...
  update(uint32_t width, uint32_t height)
  {
    ...
    uint32_t rotation = ISurfaceComposer::eRotateNone;
    status_t err = m_composer->captureScreen(m_display, m_producer, Rect(),
      width, height, 0, -1UL, false,
...
private:
  ...
  mutable sp<IGraphicBufferProducer> m_producer;
  ...
  sp<CpuConsumer> getCpuConsumer() const
  {
    if (m_cpu_consumer == NULL)
    {
      ...
      BufferQueue::createBufferQueue(&m_producer, &consumer);
      m_cpu_consumer = new CpuConsumer(consumer, 1);
