/**
 * File based on minicap_28.cpp with basically two changes:
 * -- make the SurfaceComposerClient::Transaction a global variable
 *    to overcome a segfault
 * -- dynamically call SurfaceComposerClient::getInternalDisplayToken()
 *    to replace SurfaceComposerClient::getBuiltInDisplay(int)
 */
#include "Minicap.hpp"
...
// Apparently, changing the memory layout by putting the declaration here prevents
// a segfault when compiling with android-q-preview-4.
...
minicap_try_get_display_info(int32_t displayId, Minicap::DisplayInfo* info) {
  ...
  // SurfaceComposerClient::getBuiltInDisplay(int) is no more available
  // but can be replaced by SurfaceComposerClient::getInternalDisplayToken()
  typedef android::sp<android::IBinder> (*getInternalDisplayTokenFunction)(void);
  // to prevent compilation error until AOSP get fully updated, we can dynamically load and call it
  void* handle = dlopen("libgui.so", RTLD_LAZY);
  getInternalDisplayTokenFunction getBuiltInDisplay = reinterpret_cast<getInternalDisplayTokenFunction>(
